...
public class MonsterPatch {
    /**
     * from <a href="https://steamcommunity.com/sharedfiles/filedetails/?id=2672531653">KaltsitMod</a>
     */
    public static class MonsterTakeDamagePatch {
        ...
        public static boolean gotSummon() {
            AbstractCreature c = AbstractDungeon.getMonsters().monsters.stream()
                    .filter(mo -> mo instanceof SummonedMonster).findFirst().orElse(null);
        ...
        public static class GainBlockPatch {
            public static void Prefix(GainBlockAction _inst) {
                ...
                if (_inst.target == AbstractDungeon.player && Objects.equals((Float) ReflectionHacks.getPrivate(_inst, AbstractGameAction.class, "duration"), (Float) ReflectionHacks.getPrivate(_inst, AbstractGameAction.class, "startDuration")) && MonsterTakeDamagePatch.gotSummon()) {
        ...
        public static class ChangeApplyBuffTarget {
            public static void Postfix(ApplyPowerAction _inst, AbstractCreature target, AbstractCreature source, AbstractPower power, int n, boolean b, AbstractGameAction.AttackEffect e) {
                if (!Wiz.isPlayerLing()) return; // 其他角色无效
                if (_inst != null && (source == null || !source.isPlayer) && target != null && target != source && (target == AbstractDungeon.player || power.owner == AbstractDungeon.player) && MonsterTakeDamagePatch.acceptDebuffs.contains(power.ID) && MonsterTakeDamagePatch.gotSummon()) {
                    _inst.target = MonsterTakeDamagePatch.summonTarget;
                    power.owner = _inst.target;
                    logger.info("本给予 " + target.name + " 的能力给予能力改变了目标。目前：target:" + _inst.target.name + ",owner:" + power.owner);
                }
            }
        }
